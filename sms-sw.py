...
class Alarm:
    ...
    def send_sms_alarm(self, host: str) -> None:
        for msisdn in self.msisdn_to_notify:
            url = self.sms_url.format(self.sms_user,
                                      self.sms_password,
                                      msisdn,
                                      f'Host {host} become unreachable')
            requests.post(url)
...
class DB(Alarm):
    ...
    def __init__(self,
        ...
        super().__init__(msisdn_to_notify=msisdn_to_notify,
    ...
    def compare_host_from_db_and_config(self, hosts: list) -> dict:
        # return difference between list of
        # host in existing db and hosts from parameter
    ...
    def notification_check(self,
                           notification_last_time: dt.datetime) -> bool:
        notification_delay: float = (dt.datetime.now()
                                     - notification_last_time).seconds / 60
    ...
    def check_ping_result(self, ping_result: dict) -> None:
        ...
        for host, value in ping_result.items():
            ...
            # host was unreachable
                ...
                last_notif_time: dt.datetime = dt.datetime.strptime(info['notification_last_time'], date_format)
                # check call count if it not equal count in conf
                # and call_delay more than in conf than make call
                is_notification = self.notification_check(last_notif_time)
                if info['call_count'] < self.call_count and is_notification:
                    self.send_call_alarm(host)
                    self.change_db_parameter(host,
                                             current_state=1,
                ...
                # and if it None send sms if not None - get sms_delay
                # if it equal or more than sms_delay from conf than send sms
                elif info['call_count'] == self.call_count and is_notification:
...
class Server:
    ...
    def fping_result_to_json(self, fping: str):
        ...
        for line in res:
            ...
            resp.append([dt.datetime.utcnow(),
                         'ip_from',
                         host.strip(),
                         sent,
                         received,
